...
function loadNotes() {
  ...
  .then(notes => {
    // Limpiamos el contenedor antes de agregar las notas
    $('#ct').empty();
    // Iteramos sobre las notas y las agregamos al contenedor
    notes.forEach(note => {
      const html = `
...
$('#btn-n-add').on('click', function(event) {
  const title = $('#n-title').val(); // Obtener el valor del título del formulario
  const description = $('#n-description').val(); // Obtener el valor de la descripción del formulario
  addNote(title, description);
   // Llamar a la función addNote() con los argumentos adecuados
  event.preventDefault();
/* Act on the event */
var today = new Date();
...
// Load notes when document is ready
});
